import { EmbedBuilder, ButtonBuilder, ActionRowBuilder, ButtonStyle, SlashCommandBuilder, inlineCode, ChatInputCommandInteraction } from 'discord.js';
import dayjs from 'dayjs';
import getJSONResponse from '../api/getJSONResponse.js';
import { searchMods, searchProjects } from '../api/RestClient.js';
  ...
  async execute(interaction: ChatInputCommandInteraction): Promise<void> {
    if (interaction.options.getSubcommand() === 'curseforge') {
      ...
      if (!results.data.length) {
        await interaction.editReply(`❌ No results found for **${inlineCode(query)}**`);
        return;
      }
...
function classIdToString(classId: number): string {
  switch (classId) {
    case 5:
